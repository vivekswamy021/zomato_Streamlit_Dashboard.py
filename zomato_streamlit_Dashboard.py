...
selected_type = st.sidebar.selectbox("Restaurant Type", ["All"] + sorted(data['restaurant_type'].dropna().unique()))
...
with col5:
    fig5 = px.scatter(df, x='cost_for_two', y='rate', color='online_order', hover_data=['name'], trendline="ols", title="Cost vs Rating")
...
with col8:
    ...
    fig8 = px.bar(low_voted, x='votes', y='name', orientation='h', color='votes', title="Low Voted Restaurants")
...
fig10.update_layout(title="Online + Booking")
...
- **Table Booking** improves customer perception and ratings.
- **High-cost restaurants** often maintain higher ratings.
- Many **low-vote restaurants** could benefit from better visibility and marketing.
- Focus on **Value + Quality** over price for growth.
""")
